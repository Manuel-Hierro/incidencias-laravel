...
{    
    ...
    public function install(Request $request) {
        ...
        $result = DB::unprepared(File::get(base_path() . '/IncidenciasLaravel.sql'));
        /* DB::unprepared('DROP DATABASE IF EXISTS '. $request->input('database')); */ 
        DB::unprepared('CREATE DATABASE IF NOT EXISTS ' . $request->input('database')); 
        DB::unprepared('USE ' . $request->input('database'));  
